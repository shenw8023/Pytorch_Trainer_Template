...
class HookBase:
    ...
    def every_n_iters(self, n: int) -> bool:
        return (self.trainer.iter + 1) % n == 0 if n > 0 else False  # TODO trainer.iter没有

    def every_n_inner_iters(self, n: int) -> bool:
        return (self.trainer.inner_iter + 1) % n == 0 if n > 0 else False  # +1是因为inner_iter是从0开始的，我们实际判断的时候认为第一次是1开始
